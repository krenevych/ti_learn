...
# Обери бекенд:
# ti.init(arch=ti.metal)   # macOS (M-чіпи)
# ti.init(arch=ti.cuda)  # Windows + NVIDIA
# ti.init(arch=ti.vulkan)  # AMD/Intel GPU або кросплатформено
ti.init(arch=ti.cpu)   # CPU fallback

# Додатково (необов’язково):
# ti.init(default_fp=ti.f32, default_ip=ti.i32, kernel_profiler=True)
...
# запуск
init_particles()
gui = ti.GUI("MPM Jelly", res=(1512, 1512))

while gui.running:
    for s in range(30):  # кілька субкроків
        substep()
    gui.circles(x.to_numpy(), radius=5, color=0x068587)
    gui.show()
